...
const Search = ({ allSections, onSelect, setSidebarOpen }: Props) => {
  ...
  const renderSearchList = (items: (Section & { _index?: number })[]) => (
    <ul className="p-2 pl-0 absolute top-full left-0 w-full bg-white border-t-0 rounded-b-3xl shadow-lg">
      {items.map((item, i) => {
  ...
  return (
    <div ref={containerRef} className="relative w-72 sm:w-96">
      <input
        ...
        value={
          highlightIndex >= 0 ? displayedList[highlightIndex]?.title : search
        }
